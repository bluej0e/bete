      ...
      function TorSep($tor_array){
        ...
        foreach ($gametypeTor as $gametype => $tors) {
          foreach ($tors as $key=>$tor){
            ...
            echo '<a href="https://www.bete-sport.com/tournament/'.seoUrl($currentTor['gametype'])."-".seoUrl($key).'" ><img alt="'.$key.'"src="/i/p/'. seoUrl($currentTor['gametype']).'-50.png" style="height:40px; vertical-align:text-bottom; margin-left:20px; display:inline-block;"/><h2 style="display:inline-block; width:80%;">'. $key .'</h2></a>';
