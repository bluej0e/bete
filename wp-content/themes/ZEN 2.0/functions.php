...
function gta() {
  return array('Dota 2', 'League of Legends', 'CS GO', 'Heroes of the Storm', 'Overwatch', 'Starcraft 2', 'Starcraft', 'Warcraft III', 'Hearthstone', 'Call of Duty', 'Black Ops', 'Warcraft', 'World of Tanks', 'World Of Warcraft', 'Rocket League', 'PUBG');
}
...
function TournamentSepHome($tor_array){
  foreach ($tor_array as $key=>$tor){
    ...
    echo '<div style="background: url(/i/'. seoUrl($tor[0]['gametype']).'-header.jpg) no-repeat center center" class="gametype-head"><div class="gametype-head-inner"><div></div><div><h2>'. $key .'</h2></div><div><a href="/'.seoUrl($tor[0]['gametype']).'-matches" >MORE</a></div></div></div>';
    MatchBoxes($tor);
